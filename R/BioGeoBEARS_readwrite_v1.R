...
#' @param tmplocation Default is "manual", which looks for whatever is hard-coded. 
#' Most users
#' should probably use the system.file command in the examples, below.
#' @return \code{corners_list} 
#' @export
#' @seealso \code{\link[ape]{phylo}}, \code{\link{get_nodenums}}
#' @note Go BEARS!
...
corner_coords <- function(tr, coords_fun="plot_phylo3_nodecoords", tmplocation="manual")
	...
	if (tmplocation != "manual")
		{
		extdata_dir = tmplocation
		}

	if (tmplocation == "manual")
		{
		extdata_dir = "/Dropbox/_njm/__packages/BioGeoBEARS_setup/inst/extdata/a_scripts/"
		}
	file_to_source = paste(addslash(extdata_dir), coords_fun, ".R", sep="")
